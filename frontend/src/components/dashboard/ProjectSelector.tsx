...
import {Check, ChevronDown, Circle, Folder} from 'lucide-react';
...
export default function ProjectSelector({
    ...
    if (loading) {
        return (
            <div
                className="w-48 h-10 bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] rounded-lg animate-pulse"/>
        );
    }
    ...
    if (projects.length === 0) {
        return (
            <div
                className="px-3 py-2 text-sm text-[var(--color-text-muted)] border border-dashed border-[var(--color-border-subtle)] rounded-lg">
                No projects
            </div>
        );
    }
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {selectedProject && (
                        <div className="flex items-center gap-1 text-[10px] text-[var(--color-text-dimmer)]">
                            <Circle className={`h-2 w-2 ${getStatusColor(selectedProject.status)}`}
                                    fill="currentColor"/>
                    ...
                    <>
                        ...
                        >
                            <div className="p-2 border-b border-[var(--color-border-subtle)]">
                                <div
                                    className="text-[10px] font-medium uppercase tracking-wider text-[var(--color-text-dimmer)] px-2">
                                ...
                                {projects.map((project) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div
                                                className="flex items-center gap-2 text-[10px] text-[var(--color-text-dimmer)]">
